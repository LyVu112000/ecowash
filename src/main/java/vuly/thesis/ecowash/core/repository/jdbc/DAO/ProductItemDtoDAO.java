...
public class ProductItemDtoDAO {
    ...
    private StringBuilder condition(ProductItemSearchRequest request, Map<String, Object> namedParameters) {
        ...
        conditionBuilder.append(" WHERE pi.is_other = false ");
        ...
        if (request.getProductTypeValue() != null) {
            conditionBuilder.append(" AND  prt.value = :productType ");
            namedParameters.put("productType", request.getProductTypeValue());
        }

        if (request.getProductGroupId() != null) {
            conditionBuilder.append(" AND  pi.product_group_id = :productGroup ");
            namedParameters.put("productGroup", request.getProductGroupId());
        }

        if (request.getActive() != null) {
            conditionBuilder.append(" AND  pi.active = :active ");
            namedParameters.put("active", request.getActive());
        }

        if (request.getCodeOrNameProductGroup() != null) {
            conditionBuilder.append(" AND (pr.code LIKE :codeOrName OR pr.name LIKE :codeOrName) ");
            namedParameters.put("codeOrName", "%" + request.getCodeOrNameProductGroup() + "%");
        }
